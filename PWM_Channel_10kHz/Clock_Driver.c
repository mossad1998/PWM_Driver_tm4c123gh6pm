...
void Set_Clock_MHz(int Crystal_Frequency_Mhz, int System_Clock)
{
  ...
  SYSCTL_RCC2_R &= ~(0x1FC00000);                                               //Clear Clk register
  SYSCTL_RCC2_R |= ((divisor)-1) << 22;                                         //Load Divisor value
  while((SYSCTL_RIS_R&SYSCTL_RIS_PLLLRIS)==0){};                                //wait untill take effect
  SYSCTL_RCC2_R &= ~(0x800);
}
